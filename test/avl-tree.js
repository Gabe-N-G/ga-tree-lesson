...
describe('AVLTree', ()=>{
    ...
    it('should automatically fix a right right imbalance further down the tree', ()=> {
        let tree = new AVLTree();
        tree.insert(4);
        tree.insert(3);
        tree.insert(5);
        ...
        expect(tree.root.right.height).to.equal(2);
        expect(tree.root.height).to.equal(3);
    })
    it('should automatically fix a right left imbalance further down the tree', ()=> {
        let tree = new AVLTree();
        tree.insert(4);
        tree.insert(3);
        tree.insert(7);
        tree.insert(10);
        tree.insert(8);
        expect(tree.root.right.data).to.equal(8);
        expect(tree.root.right.left.data).to.equal(7);
        expect(tree.root.right.right.data).to.equal(10);
        expect(tree.root.right.height).to.equal(2);
        expect(tree.root.right.left.height).to.equal(1);
    })
})